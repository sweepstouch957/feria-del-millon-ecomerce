...
export default function GenericLoginPage() {
  ...
  const [role, setRole] = useState<Role>(["artist", "buyer"].includes(roleParam) ? roleParam : "buyer");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try { window.sessionStorage.setItem("LOGIN_NEXT", ui.next); } catch {}
      ...
      <div className="w-full max-w-md">
            ...
            <div className="mt-4 inline-flex items-center rounded-full border border-gray-200">
              <Link
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-l-full transition ${
                  role === "buyer" ? "bg-black text-white" : "text-gray-700 hover:bg-gray-100"
                }`}
              ...
              <Link
                ...
                className={`px-3 py-1.5 text-xs font-medium rounded-r-full transition ${
                  role === "artist" ? "bg-black text-white" : "text-gray-700 hover:bg-gray-100"
                }`}
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 text-center">
            {role === "artist" ? (
              <p className="text-sm text-gray-700">
                ...
                <Link href="/artist/registro" className="font-medium text-black underline underline-offset-4 hover:no-underline">
                  Crear cuenta
                </Link>
              </p>
            ) : (
              <p className="text-sm text-gray-700">
                ¿Nuevo en la feria?{" "}
                <Link href="/registro" className="font-medium text-black underline underline-offset-4 hover:no-underline">
                  Crear cuenta
                </Link>
              </p>
            )}
        ...
        <div className="text-center mt-6">
          <p className="text-xs text-gray-500">
            2025 • Feria del Millón — {role === "artist" ? "Portal de Artistas" : "Compradores"}
          </p>
        </div>
      </div>
    </div>
